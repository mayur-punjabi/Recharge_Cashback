...
import amazon.login.collectbeautyoffer.CollectBeautyOffer;
import suites.basesuite.BaseSuite;

public class CollectBeautyOfferSuite extends BaseSuite {
	...
	public void collectOffer(String isParallel, String starting, String ending, String statusFilePath) {
		...
		String csvFilePath = "./excel/collectbeautyoffer.csv";
		...
		if (Boolean.parseBoolean(isParallel)) {
			...
			statusFilePath = cf.createStatusFile("collectbeautyoffer");
		}
		...
		for (int i = startIndex - 1; i < endIndex; i++) {
			...
			try {
				...
				if (failure.isEmpty()) {
					...
					CollectBeautyOffer collectBeautyOffer = new CollectBeautyOffer();
					failure = collectBeautyOffer.collectOffer(offerURL);
				}
