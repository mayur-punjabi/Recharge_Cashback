...
import amazon.login.addaddress.AddAddress;
import suites.basesuite.BaseSuite;

public class AddAddressSuite extends BaseSuite {
	...
	public void gvCashback(String isParallel, String starting, String ending, String statusFilePath) {
		...
		String csvFilePath = "./excel/addAddress.csv";
		...
		if (Boolean.parseBoolean(isParallel)) {
			...
			statusFilePath = cf.createStatusFile("addAddress");
		}
		...
		for (int i = startIndex - 1; i < endIndex; i++) {
			...
			String mobile = line.get(3);
			String pincode = line.get(4);
			String flat = line.get(5);
			String area = line.get(6);
			...
			try {
				...
				if (failure.isEmpty()) {
					...
					failure = addAddress.launchAndAddAddress(name, mobile, pincode, flat, area);
				}
				cf.logout();
			} catch (Exception e) {
			...
			cf.updateStatusFile(statusFilePath, line, failure, 7);
