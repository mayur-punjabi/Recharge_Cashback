...
import amazon.login.addmoney.AddMoney;
import suites.basesuite.BaseSuite;

public class AddMoneySuite extends BaseSuite {
	...
	public void cashback(String isParallel, String starting, String ending, String statusFilePath) {
		...
		String csvFilePath = "./excel/addMoney.csv";
		...
		if (Boolean.parseBoolean(isParallel)) {
			...
			statusFilePath = cf.createStatusFile("addMoney");
		}
		...
		for (int i = startIndex - 1; i < endIndex; i++) {
			...
			String voterID = line.get(2);
			String amount = line.get(3);
			String name = line.get(4);
			String mobile = line.get(5);
			String pincode = line.get(6);
			String flat = line.get(7);
			String area = line.get(8);
			...
			try {
				...
				if (failure.isEmpty()) {
					...
					failure = addMoney.addMoney(voterID, amount, name, mobile, pincode, flat, area, password, "card");
				}
			...
			cf.updateStatusFile(statusFilePath, line, failure, 9);
