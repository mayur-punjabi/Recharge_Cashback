...
import amazon.login.collectcardoffer.CollectCardOffer;
import suites.basesuite.BaseSuite;

public class CollectCardOfferSuite extends BaseSuite {
	...
	public void collectOffer(String isParallel, String starting, String ending, String statusFilePath) {
		...
		String csvFilePath = "./excel/collectCardOffer.csv";
		...
		if (Boolean.parseBoolean(isParallel)) {
			...
			statusFilePath = cf.createStatusFile("collectCardOffer");
		}
		...
		for (int i = startIndex - 1; i < endIndex; i++) {
			...
			String cardNo = line.get(2);
			String cardName = line.get(3);
			String month = line.get(4);
			String year = line.get(5);
			String cvv = line.get(6);
			String email = line.get(7);
			String pin = line.get(8);
			...
			try {
				...
				if (failure.isEmpty()) {
					...
					CollectCardOffer collectCardOffer = new CollectCardOffer();
					failure = collectCardOffer.collectCardOffer(cardNo, cardName, month, year, cvv, email, pin);
				}
