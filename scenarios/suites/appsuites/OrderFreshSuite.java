...
import amazon.login.orderproduct.OrderFresh;
import suites.basesuite.BaseSuite;

public class OrderFreshSuite extends BaseSuite {
	...
	public void orderProduct(String isParallel, String starting, String ending, String statusFilePath) {
		...
		String csvFilePath = "./excel/orderfresh.csv";
		...
		if (Boolean.parseBoolean(isParallel)) {
			...
			statusFilePath = cf.createStatusFile("orderfresh");
		}
		...
		for (int i = startIndex - 1; i < endIndex; i++) {
			...
			try {
				...
				if (failure.isEmpty()) {
					...
					if (failure.isEmpty()) {
						...
						failure = orderFresh.launchAndOrderProduct(name, mobile, pincode, flat, area, gv);
					}
				}
			...
			cf.updateStatusFile(statusFilePath, line, failure, 11);
