...
import amazon.login.electricity.Electricity;
import amazon.login.giftcard.GiftCard;
import suites.basesuite.BaseSuite;

public class ElectricitySuite extends BaseSuite {

	@Test(priority = 0)
	@Parameters({ "isParallel", "starting", "ending", "statusFilePath" })
	public void electricity(String isParallel, String starting, String ending, String statusFilePath) {

		String csvFilePath = "./excel/electricity.csv";
		...
		if (Boolean.parseBoolean(isParallel)) {
			...
			statusFilePath = cf.createStatusFile("electricity");
		}
		...
		for (int i = startIndex - 1; i < endIndex; i++) {
			...
			String state = line.get(3);
			String board = line.get(4);
			String id = line.get(5);
			String amount = line.get(6);
			String name = line.get(7);
			String mobile = line.get(8);
			String pincode = line.get(9);
			String flat = line.get(10);
			String area = line.get(11);
			String checkOffer = line.get(12);
			...
			try {
				...
				if (failure.isEmpty()) {
					...
					failure = electricity.launchAndCheckOffer(checkOffer);
					...
					if (failure.isEmpty()) {
						...
						// recharge
						if (failure.isEmpty()) {
							failure = electricity.launchAndRechargeElectricity(state, board, id, amount, name, mobile,
									pincode, flat, area, password, gv);
						}
					}
				}
			...
			cf.updateStatusFile(statusFilePath, line, failure, 13);
